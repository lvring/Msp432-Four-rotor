...
// Name, Clock               , AF-UART      ,UART  , Baud, Interrupt
  {
	...
	// PORT , PIN      , Clock              , Source
		{GPIOB,GPIO_Pin_10,RCC_AHB1Periph_GPIOB,GPIO_PinSource10},  // TX an PD8
...
// Name, Clock               , AF-UART      ,UART  , Baud, Interrupt
  {
		COM_6,RCC_APB2Periph_USART6,GPIO_AF_USART6,USART6,115200,USART6_IRQn, // UART3 mit 115200 Baud
	// PORT , PIN      , Clock              , Source
		{GPIOC,GPIO_Pin_6,RCC_AHB1Periph_GPIOC,GPIO_PinSource6},  // TX an PD8
		{GPIOC,GPIO_Pin_7,RCC_AHB1Periph_GPIOC,GPIO_PinSource7},
	//BufferLen QUENELEN 
		32,64,
	}, // RX an PD9
};
...
void BSP_COM_Init(void)
{
  ...
  for(nr=(BSP_COM_Name_e)0;nr<COM_N;nr++) {
    ...
    // RX-Puffer vorbereiten
		//申请内存
		ComControlArray[nr].ComBuffer.puint8_tDatabuf = (uint8_t *)RTE_BGetz(MEM_RTE,ComControlArray[nr].DataBufferLen);
	...
	//创建定时器
	RTE_RoundRobin_CreateTimer("COMTimer",20,0,0,0,COMTimer_Callback,&ComTimerID);
}
//--------------------------------------------------------------
...
extern volatile uint16_t RotateAngle;
static void COMTimer_Callback(void* arg)
{
	...
	{
		if(* com_name != COM_3)
		RTE_MessageQuene_In(&ComControlArray[*com_name].ComBuffer.ComQuene,ComControlArray[*com_name].ComBuffer.puint8_tDatabuf,
				ComControlArray[*com_name].ComBuffer.u16Datalength);
		else
			RotateAngle = atof(ComControlArray[*com_name].ComBuffer.puint8_tDatabuf);
	}
	memset(ComControlArray[*com_name].ComBuffer.puint8_tDatabuf,0,ComControlArray[*com_name].DataBufferLen);
	ComControlArray[*com_name].ComBuffer.u16Datalength = 0;
}
static void BSP_COM_RecCallback(uint16_t byte,BSP_COM_Name_e com_name)
{
	ComTimerID = com_name;
	ComControlArray[com_name].ComBuffer.puint8_tDatabuf[ComControlArray[com_name].ComBuffer.u16Datalength++] = byte;
...
void USART6_IRQHandler(void) {
  ...
  if (USART_GetITStatus(USART6, USART_IT_RXNE) == SET) {
    ...
    // Byte speichern
		BSP_COM_RecCallback(wert,COM_6);
  }
}
