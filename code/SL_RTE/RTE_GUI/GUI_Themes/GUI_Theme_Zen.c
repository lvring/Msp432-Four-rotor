...
{
    static style_t bg;
    static style_t panel;

    style_copy(&def, &style_pretty);  /*Initialize the default style*/
    def.body.border.opa = GUI_OPA_COVER;
    def.text.font = _font;
    def.text.color = GUI_COLOR_HEX3(0x444);

    style_copy(&bg, &def);
    bg.body.main_color = GUI_COLOR_WHITE;
    bg.body.grad_color = GUI_COLOR_WHITE;
    bg.body.radius = 0;
    ...
    panel.body.border.color = color_hsv_to_rgb(_hue, 30, 90);
    panel.body.border.opa = GUI_OPA_COVER;
    panel.body.shadow.width = 4;
    panel.body.shadow.color = GUI_COLOR_HEX3(0xddd);
    panel.body.padding.hor = GUI_DPI / 6;
    panel.body.padding.ver = GUI_DPI / 8;
    panel.body.padding.inner = GUI_DPI / 10;

    style_copy(&sb, &def);
    sb.body.main_color = color_hsv_to_rgb(_hue, 30, 90);
...
{
#if GUI_USE_BTN != 0
    static style_t rel, pr, tgl_pr, ina;
    style_copy(&rel, &def);
    rel.body.empty = 1;
    rel.body.radius = GUI_RADIUS_CIRCLE;
    rel.body.border.width = 2;
    rel.body.border.color = color_hsv_to_rgb(_hue, 40, 90);
    rel.body.border.opa = GUI_OPA_COVER;
    rel.body.shadow.width = 4;
    rel.body.shadow.color = GUI_COLOR_HEX3(0xddd);
    rel.body.padding.hor = GUI_DPI / 4;
    rel.body.padding.ver = GUI_DPI / 8;
    rel.text.color = color_hsv_to_rgb(_hue, 40, 90);

    style_copy(&pr, &rel);
    pr.body.border.color = color_hsv_to_rgb(_hue, 40, 60);
    pr.text.color = color_hsv_to_rgb(_hue, 40, 60);
    pr.body.shadow.width = 0;

    style_copy(&tgl_pr, &pr);
    tgl_pr.body.border.color = color_hsv_to_rgb(_hue, 40, 50);
    tgl_pr.text.color = color_hsv_to_rgb(_hue, 40, 50);

    style_copy(&ina, &tgl_pr);
    ina.body.border.color = GUI_COLOR_HEX3(0xbbb);
    ina.text.color = GUI_COLOR_HEX3(0xbbb);
...
static void label_init(void)
{
    ...
    prim.text.color = GUI_COLOR_HEX3(0x555);
    sec.text.color = color_hsv_to_rgb(_hue, 50, 80);
    hint.text.color = color_hsv_to_rgb(_hue, 25, 85);
...
static void img_init(void)
{
    ...
    img_light.image.color = color_hsv_to_rgb(_hue, 85, 55);
    img_light.image.intense = GUI_OPA_80;
...
static void led_init(void)
{
    ...
    led.body.main_color = color_hsv_to_rgb(_hue, 60, 100);
    led.body.grad_color = color_hsv_to_rgb(_hue, 60, 40);
    led.body.border.color = color_hsv_to_rgb(_hue, 60, 60);
    led.body.shadow.color = color_hsv_to_rgb(_hue, 80, 100);
...
{
#if GUI_USE_BAR
    static style_t bg, indic;

    style_copy(&bg, &def);
    bg.body.empty = 1;
    bg.body.radius = GUI_RADIUS_CIRCLE;
    bg.body.border.width = 2;
    bg.body.border.opa = GUI_OPA_COVER;
    bg.body.border.color = color_hsv_to_rgb(_hue, 40, 90);

    style_copy(&indic, &def);
    indic.body.radius = GUI_RADIUS_CIRCLE;
    indic.body.main_color = color_hsv_to_rgb(_hue, 40, 90);
    indic.body.grad_color = indic.body.main_color;
    indic.body.border.width = 0;
    indic.body.padding.hor = GUI_DPI / 20;
    indic.body.padding.ver = GUI_DPI / 20;
...
{
#if GUI_USE_SW != 0
    static style_t indic;

    style_copy(&indic, theme.slider.indic);
    indic.body.radius = GUI_RADIUS_CIRCLE;
    indic.body.main_color = color_hsv_to_rgb(_hue, 15, 95);
    indic.body.grad_color = indic.body.main_color;
...
static void lmeter_init(void)
{
    ...
    lmeter.line.width = 2;
    lmeter.body.main_color = color_hsv_to_rgb(_hue, 80, 70);
    lmeter.body.grad_color = lmeter.body.main_color;
    lmeter.body.padding.hor = GUI_DPI / 8;
...
static void gauge_init(void)
{
    ...
    gauge.line.color = color_hsv_to_rgb(_hue, 50, 70);
    gauge.line.width = 1;
    gauge.body.main_color = GUI_COLOR_HEX3(0x999);
    gauge.body.grad_color = gauge.body.main_color;
    gauge.body.padding.hor = GUI_DPI / 16;
    gauge.body.border.color = GUI_COLOR_HEX3(0x666);     /*Needle middle color*/
...
static void cb_init(void)
{
    ...
    rel.body.border.opa = GUI_OPA_COVER;
    rel.body.border.color = color_hsv_to_rgb(_hue, 35, 80);
    rel.body.main_color = GUI_COLOR_HEX3(0xfff);
    rel.body.grad_color = rel.body.main_color;


    style_copy(&pr, &rel);
    pr.body.border.color = color_hsv_to_rgb(_hue, 35, 70);


    style_copy(&tgl_rel, &rel);
    tgl_rel.body.border.color = color_hsv_to_rgb(_hue, 45, 80);
    tgl_rel.body.main_color = color_hsv_to_rgb(_hue, 40, 90);
    tgl_rel.body.grad_color = tgl_rel.body.main_color;

    style_copy(&tgl_pr, &rel);
    tgl_pr.body.border.color = color_hsv_to_rgb(_hue, 45, 70);
    tgl_pr.body.main_color = color_hsv_to_rgb(_hue, 40, 80);
    tgl_pr.body.grad_color = tgl_pr.body.main_color;


    style_copy(&ina, &rel);
    ina.body.border.color = GUI_COLOR_HEX3(0xaaa);
...
static void btnm_init(void)
{
    ...
    style_copy(&bg, &style_transp);
    bg.glass = 0;
    bg.body.padding.hor = 0;
    bg.body.padding.ver = 0;
    bg.body.padding.inner = GUI_DPI / 15;
    bg.text.font = _font;
    ...
    tgl_rel.body.main_color = color_hsv_to_rgb(_hue, 15, 95);
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.body.border.width = 0;
    tgl_rel.text.color = color_hsv_to_rgb(_hue, 60, 40);

    style_copy(&tgl_pr, &tgl_rel);
    tgl_pr.body.main_color = color_hsv_to_rgb(_hue, 30, 70);
    tgl_pr.body.grad_color = tgl_pr.body.main_color;

    style_copy(&ina, &pr);
    ina.body.main_color = GUI_COLOR_HEX3(0x888);
    ina.body.grad_color = tgl_pr.body.main_color;
    ina.text.color = GUI_COLOR_HEX3(0x888);;
...
static void kb_init(void)
{
    ...
    static style_t bg, rel ,pr, tgl_rel, tgl_pr, ina;
    style_copy(&bg, &def);
     bg.body.main_color = GUI_COLOR_HEX3(0x666);
    ...
    rel.body.border.color = GUI_COLOR_HEX3(0x888);
    rel.body.border.opa = GUI_OPA_COVER;
    rel.text.color = GUI_COLOR_WHITE;

    style_copy(&pr, &def);
    pr.body.main_color = GUI_COLOR_HEX3(0xeee);
    pr.body.grad_color = pr.body.main_color;
    pr.body.border.color = GUI_COLOR_HEX3(0x888);
    pr.body.border.width = 1;
    pr.body.border.opa = GUI_OPA_COVER;
    pr.body.radius = 0;
    pr.text.color = GUI_COLOR_HEX3(0x666);

    style_copy(&tgl_rel, &pr);
    tgl_rel.body.main_color = GUI_COLOR_HEX3(0x999);
    tgl_rel.body.grad_color = tgl_rel.body.main_color;
    tgl_rel.text.color = GUI_COLOR_WHITE;


    style_copy(&tgl_pr, &pr);
    tgl_pr.body.main_color = GUI_COLOR_HEX3(0xbbb);
    tgl_pr.body.grad_color = tgl_pr.body.main_color;
    tgl_pr.text.color = GUI_COLOR_HEX3(0xddd);
...
{
#if GUI_USE_MBOX
    static style_t bg, rel, pr;
    style_copy(&bg, theme.panel);
    bg.body.main_color = color_hsv_to_rgb(_hue, 10, 95);
    bg.body.grad_color = bg.body.main_color;
    bg.text.color = color_hsv_to_rgb(_hue, 40, 25);

    style_copy(&rel, &def);
    rel.body.main_color = color_hsv_to_rgb(_hue, 25, 85);
    rel.body.grad_color = rel.body.main_color;
    rel.body.radius = GUI_RADIUS_CIRCLE;
    rel.body.border.width = 2;
    rel.body.border.color = color_hsv_to_rgb(_hue, 30, 70);
    rel.body.padding.hor = GUI_DPI / 4;
    rel.body.padding.ver = GUI_DPI / 8;
    rel.text.color = bg.text.color;

    style_copy(&pr, &rel);
    pr.body.border.color = color_hsv_to_rgb(_hue, 30, 90);
    pr.text.color = color_hsv_to_rgb(_hue, 40, 40);
    pr.body.main_color = color_hsv_to_rgb(_hue, 20, 85);
    pr.body.grad_color = pr.body.main_color;
...
static void ta_init(void)
{
    ...
    style_copy(&oneline, theme.panel);
    oneline.body.radius = GUI_RADIUS_CIRCLE;
    oneline.body.padding.ver = GUI_DPI / 10;
...
static void list_init(void)
{
    ...
    rel.body.padding.ver = GUI_DPI / 8;
    rel.text.color = GUI_COLOR_HEX3(0x666);

    style_copy(&pr, &rel);
    pr.text.color = theme.btn.pr->text.color;

    style_copy(&tgl_rel, &rel);
    tgl_rel.text.color = color_hsv_to_rgb(_hue, 50, 90);
...
static void ddlist_init(void)
{
    ...
    bg.body.padding.ver = GUI_DPI / 8;
    bg.text.color = GUI_COLOR_HEX3(0x666);

    style_copy(&sel, &def);
    sel.body.empty = 1;
    sel.body.border.width = 0;
    sel.text.color = color_hsv_to_rgb(_hue, 50, 80);
...
static void roller_init(void)
{
    ...
    bg.body.border.width = 0;
    bg.body.empty = 1;
    bg.text.line_space = GUI_DPI / 6;
    bg.text.color = GUI_COLOR_HEX3(0x999);

    style_copy(&sel, theme.panel);
    sel.body.radius = GUI_RADIUS_CIRCLE;
    sel.body.empty = 1;
...
static void tabview_init(void)
{
    ...
    static style_t btn_bg, indic, rel, pr, tgl_rel, tgl_pr;

    style_copy(&btn_bg, &def);
    btn_bg.body.empty = 1;
    btn_bg.body.border.width = 2;
    btn_bg.body.border.part = GUI_BORDER_BOTTOM;
    btn_bg.body.border.color = color_hsv_to_rgb(_hue, 10, 90);

    style_copy(&indic, &def);
    indic.body.padding.inner = GUI_DPI / 16;
    indic.body.border.width = 0;
    indic.body.radius = GUI_RADIUS_CIRCLE;
    indic.body.main_color = color_hsv_to_rgb(_hue, 50, 80);
    indic.body.grad_color = indic.body.main_color;

    style_copy(&rel, &def);
    rel.body.empty = 1;
    rel.body.border.width = 0;
    rel.text.color = GUI_COLOR_HEX3(0x999);


    style_copy(&pr, &rel);
    pr.text.color = GUI_COLOR_HEX3(0x777);

    style_copy(&tgl_rel, &rel);
    tgl_rel.text.color = color_hsv_to_rgb(_hue, 50, 80);

    style_copy(&tgl_pr, &rel);
    tgl_pr.text.color = color_hsv_to_rgb(_hue, 50, 70);
...
{
#if GUI_USE_WIN != 0
    static style_t header, rel, pr;

    style_copy(&header, &def);
    header.body.empty = 1;
    header.body.border.width = 2;
    header.body.border.part = GUI_BORDER_BOTTOM;
    header.body.border.color = color_hsv_to_rgb(_hue, 10, 90);
    header.text.color = GUI_COLOR_HEX3(0x666);
    ...
    style_copy(&pr, &rel);
    pr.text.color = GUI_COLOR_HEX3(0x333);
