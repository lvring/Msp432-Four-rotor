...
typedef struct
{
    ...
    ll_t series_ll;       /*Linked list for the data line pointers (stores chart_dl_t)*/
    coord_t ymin;          /*y min value (used to scale the data)*/
    coord_t ymax;          /*y max value (used to scale the data)*/
    uint8_t hdiv_cnt;     /*Number of horizontal division lines*/
    uint8_t vdiv_cnt;     /*Number of vertical division lines*/
    uint16_t point_cnt;   /*Point number in a data line*/
    uint8_t type    :3;   /*Line, column or point chart (from 'chart_type_t')*/
    struct {
        coord_t width;  /*Line width or point radius*/
...
typedef enum
{
	GUI_CHART_TYPE_LINE = 0x01,
	GUI_CHART_TYPE_COLUMN = 0x02,
	GUI_CHART_TYPE_POINT = 0x04,
}chart_type_t;
 ...
 */
static inline void chart_set_style(obj_t *chart, style_t *style)
{
    obj_set_style(chart, style);
}
 ...
 */
static inline style_t* chart_get_style(obj_t *chart)
{
    return obj_get_style(chart);
}
