 ...
 *      DEFINES
 *********************/
#define GUI_LABEL_DOT_NUM    3
#define GUI_LABEL_POS_LAST   0xFFFF

/**********************
 *      TYPEDEFS
 **********************/

/*Long mode behaviors. Used in 'label_ext_t' */
typedef enum
{
    GUI_LABEL_LONG_EXPAND,   /*Expand the object size to the text size*/
    GUI_LABEL_LONG_BREAK,    /*Keep the object width, break the too long lines and expand the object height*/
    GUI_LABEL_LONG_SCROLL,   /*Expand the object size and scroll the text on the parent (move the label object)*/
    GUI_LABEL_LONG_DOT,      /*Keep the size and write dots at the end if the text is too long*/
    GUI_LABEL_LONG_ROLL,     /*Keep the size and roll the text infinitely*/
}label_long_mode_t;
...
typedef struct
{
    ...
    label_long_mode_t long_mode; /*Determinate what to do with the long texts*/
#if GUI_TXT_UTF8 == 0
    char dot_tmp[GUI_LABEL_DOT_NUM + 1]; /*Store the character which are replaced by dots (Handled by the library)*/
#else
    char dot_tmp[GUI_LABEL_DOT_NUM * 4 + 1]; /*Store the character which are replaced by dots (Handled by the library)*/
#endif
    uint16_t dot_end;               /*The text end position in dot mode (Handled by the library)*/
    uint16_t anim_speed;            /*Speed of scroll and roll animation in px/sec unit*/
    point_t offset;                 /*Text draw position offset*/
    uint8_t static_txt  :1;         /*Flag to indicate the text is static*/
    uint8_t align       :2;         /*Align type from 'label_align_t'*/
    uint8_t recolor     :1;         /*Enable in-line letter re-coloring*/
    uint8_t expand      :1;         /*Ignore real width (used by the library with GUI_LABEL_LONG_ROLL)*/
 ...
 */
static inline void label_set_style(obj_t *label, style_t *style)
{
    obj_set_style(label, style);
}
/*=====================
 * Getter functions
 *====================*/
 ...
 */
static inline style_t* label_get_style(obj_t *label)
{
    return obj_get_style(label);
}
