...
void txt_get_size(point_t * size_res, const char * text, const font_t * font,
                    coord_t letter_space, coord_t line_space, coord_t max_width, txt_flag_t flag)
{
    ...
    if(flag & GUI_TXT_FLAG_EXPAND) max_width = GUI_COORD_MAX;

    uint32_t line_start = 0;
    uint32_t new_line_start = 0;
    coord_t act_line_length;
    ...
    while (text[line_start] != '\0') {
        ...
        size_res->x = RTE_MATH_MAX(act_line_length, size_res->x);
...
uint16_t txt_get_next_line(const char * txt, const font_t * font,
                           coord_t letter_space, coord_t max_width, txt_flag_t flag)
{
    ...
    while(txt[i] != '\0') {
        ...
        if((flag & GUI_TXT_FLAG_NO_BREAK) == 0 && (letter == '\n' || letter == '\r')) {
            /*Handle \r\n as well*/
            uint32_t i_tmp = i;
            ...
            cur_w += font_get_width(font, letter);
            ...
            if(cur_w > max_width) {
                ...
                if(last_break != NO_BREAK_FOUND ) {
                    ...
                    /* But 'i' already points to the next character (because of txt_utf8_next) step beck one*/
                    txt_utf8_prev(txt, &i);
                }
 ...
 */
coord_t txt_get_width(const char * txt, uint16_t length, 
                            const font_t * font, coord_t letter_space, txt_flag_t flag)
{
    if(txt == NULL) return 0;
    if(font == NULL) return 0;
    ...
    if(length != 0) {
        while(i < length) {
            ...
            width += font_get_width(font, letter);
            width += letter_space;
        }
        ...
        for(i = length - 1; i > 0; i--) {
            if(txt[i] == ' ') {
                width -= font_get_width(font, txt[i]);
                width -= letter_space;
            } else {
                break;
            }
...
bool txt_is_cmd(txt_cmd_state_t * state, uint32_t c)
{
    ...
    if(c == (uint32_t)GUI_TXT_COLOR_CMD[0]) 
		{
      if(*state == GUI_TXT_CMD_STATE_WAIT) 
			{ 
						//Start char
           *state = GUI_TXT_CMD_STATE_PAR;
           ret = true;
      } 
			else if(*state == GUI_TXT_CMD_STATE_PAR) 
			{ 
				//Other start char in parameter is escaped cmd. char 
				 *state = GUI_TXT_CMD_STATE_WAIT;
      }
			//Command end
			 else if(*state == GUI_TXT_CMD_STATE_IN) { 
				 *state = GUI_TXT_CMD_STATE_WAIT;
				 ret = true;
       }
   }
   ...
   if(*state == GUI_TXT_CMD_STATE_PAR) {
       if(c == ' ') {
           *state = GUI_TXT_CMD_STATE_IN; /*After the parameter the text is in the command*/
       }
       ret = true;
   }
 ...
 */
uint32_t txt_get_length(const char * txt)
{
#if GUI_TXT_UTF8 == 0
    return strlen(txt);
#else
...
static bool is_break_char(uint32_t letter)
{
    ...
    for(i = 0; GUI_TXT_BREAK_CHARS[i] != '\0'; i++) {
        if(letter == (uint32_t)GUI_TXT_BREAK_CHARS[i]) {
