...
typedef enum
{
    GUI_TXT_FLAG_NONE =     0x00,
    GUI_TXT_FLAG_RECOLOR =  0x01,   /*Enable parsing of recolor command*/
    GUI_TXT_FLAG_EXPAND =   0x02,   /*Ignore width (Used by the library)*/
    GUI_TXT_FLAG_NO_BREAK = 0x04,   /*Ignore line breaks (Used by the library)*/
    GUI_TXT_FLAG_CENTER =   0x08,   /*Align the text to the middle*/
}txt_flag_t;

typedef enum
{
    GUI_TXT_CMD_STATE_WAIT,      /*Waiting for command*/
    GUI_TXT_CMD_STATE_PAR,       /*Processing the parameter*/
    GUI_TXT_CMD_STATE_IN,        /*Processing the command*/
}txt_cmd_state_t;
...
uint16_t txt_get_next_line(const char * txt, const font_t * font_p,
                            coord_t letter_space, coord_t max_l, txt_flag_t flag);
 ...
 */
coord_t txt_get_width(const char * txt, uint16_t char_num,
                    const font_t * font_p, coord_t letter_space, txt_flag_t flag);
